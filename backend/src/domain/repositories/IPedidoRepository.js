/**
 * IPedidoRepository Interface
 */
class IPedidoRepository {
    async save(pedido) { throw new Error('Method not implemented'); }
    async findById(id) { throw new Error('Method not implemented'); }
    async findAll(filtros) { throw new Error('Method not implemented'); }
    async findByClienteId(clienteId, filtros) { throw new Error('Method not implemented'); }
    async count(filtros) { throw new Error('Method not implemented'); }
    async update(id, data) { throw new Error('Method not implemented'); }
    async getEstadisticas(barberiaId, fechaInicio, fechaFin) { throw new Error('Method not implemented'); }
}

module.exports = IPedidoRepository;
